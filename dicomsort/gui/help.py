...
Version 1.1 now provides the user the ability to remove sensitive patient data from
the DICOM metadata. To do this, select the <strong>Anonymize</strong> checkbox.
Pressing this button will bring up a window that will allow you to select which fields
you do not want included in the output images. You can always revert back to the default
properties using the <strong>Revert to Defaults</strong> button.
<br><br>
If you wish to change the properties that are considered to be private by default,
you can edit the anon.inc file located in the application directory. Just edit this file
using any text editor and make sure to include one field per line in the file. The next
time the program is loaded, these fields will be used as the default anonymous fields.
<br><br>
Another option is to use the original filenames as the input files rather than renaming
based upon the image type. This plays an important role if the analysis program to be used
requires a certain naming convention. Names will be preserved when the checkbox is ticked.
<br><br>
